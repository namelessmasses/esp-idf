    ...
    for (int32_t y = y1; y <= y2; ++y) {
        ...
        for (int32_t x = x1; x <= x2; ++x) {
            ...
            const uint8_t reset_set_bit_in_byte[] = {
                (uint8_t)(s_SH1106_framebuffer[sh1106_byte_offset_for_pixel] &
...
static struct {
    ...
    .layout_column_dsc = {64, 64, LV_GRID_TEMPLATE_LAST},
...
void ui_run(i2c_master_bus_handle_t bus_handle) {
    ...
    lv_obj_set_grid_cell(
        s_ui.temp, LV_GRID_ALIGN_START, 0, 1, LV_GRID_ALIGN_CENTER, 0, 1);

    s_ui.rh = lv_label_create(scr);
    lv_obj_add_style(s_ui.rh, &s_ui.style, LV_PART_MAIN);
    lv_label_set_text(s_ui.rh, "--.--%");
    lv_obj_set_grid_cell(
        s_ui.rh, LV_GRID_ALIGN_START, 1, 1, LV_GRID_ALIGN_CENTER, 0, 1);
...
#endif
    lv_obj_set_grid_cell(
        s_ui.voltage, LV_GRID_ALIGN_CENTER, 0, 1, LV_GRID_ALIGN_CENTER, 1, 1);
